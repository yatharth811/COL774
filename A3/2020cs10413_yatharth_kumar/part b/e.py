...
if __name__ == '__main__':
    ...
    networks = [NeuralNetwork(X = X_train, Y = y_train_onehot, layers = network, activation = lambda z: np.maximum(0, z), activation_derivative = lambda z: 1. * (z > 0), mini_batch_size=32, learning_rate=0.01, epochs=500, adaptive=True) for network in networks]
    ...
    plot_metrics([1, 2, 3, 4], f1_scores, accuracies, "c_test.png")
    
    f1_scores, accuracies = [], []
    for network in networks:
        f1, acc = get_score(network, X_train, y_train_onehot)
        f1_scores.append(f1)
        accuracies.append(acc)
        
    plot_metrics([1, 2, 3, 4], f1_scores, accuracies, "c_train.png")